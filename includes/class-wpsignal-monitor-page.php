...
class Monitor_Page {
	...
	public function render_page() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'WPSignal Monitor', 'eventra-for-wpsignal' ); ?></h1>
	...
	private function render_connection_status() {
		...
		<div class="card" style="max-width:100%;">
			<h2><?php esc_html_e( 'Connection Status', 'eventra-for-wpsignal' ); ?></h2>

			<p>
				<?php if ( $configured ) : ?>
					<span class="dashicons dashicons-yes-alt" style="color:#46b450;"></span>
					<strong><?php esc_html_e( 'Configured', 'eventra-for-wpsignal' ); ?></strong>
				<?php else : ?>
					<span class="dashicons dashicons-dismiss" style="color:#dc3232;"></span>
					<strong><?php esc_html_e( 'Not Configured', 'eventra-for-wpsignal' ); ?></strong>
					&mdash; <?php esc_html_e( 'Go to Settings to connect.', 'eventra-for-wpsignal' ); ?>
				<?php endif; ?>
			</p>
			...
			<?php if ( $configured ) : ?>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Server URL', 'eventra-for-wpsignal' ); ?></th>
						<td><code><?php echo esc_html( $base_url ); ?></code></td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Site Key', 'eventra-for-wpsignal' ); ?></th>
				...
				<p>
					<button type="button" id="wpsignal-ks-test-connection" class="button">
						<?php esc_html_e( 'Test Connection', 'eventra-for-wpsignal' ); ?>
					</button>
	...
	private function render_triggers_table( $triggers ) {
		...
		<div class="card" style="max-width:100%;margin-top:20px;">
			<h2><?php esc_html_e( 'Registered Triggers', 'eventra-for-wpsignal' ); ?></h2>

			<?php if ( empty( $triggers ) ) : ?>
				<p><?php esc_html_e( 'No triggers registered.', 'eventra-for-wpsignal' ); ?></p>
			<?php else : ?>
				<table class="widefat striped">
					<thead>
						<tr>
							<th><?php esc_html_e( 'Event', 'eventra-for-wpsignal' ); ?></th>
							<th><?php esc_html_e( 'Hook', 'eventra-for-wpsignal' ); ?></th>
							<th><?php esc_html_e( 'Priority', 'eventra-for-wpsignal' ); ?></th>
							<th><?php esc_html_e( 'Args', 'eventra-for-wpsignal' ); ?></th>
							<th><?php esc_html_e( 'Channel', 'eventra-for-wpsignal' ); ?></th>
							<th><?php esc_html_e( 'Condition', 'eventra-for-wpsignal' ); ?></th>
						</tr>
					</thead>
					<tbody>
	...
	private function render_event_log() {
		?>
		<div class="card" style="max-width:100%;margin-top:20px;">
			<h2><?php esc_html_e( 'Live Event Log', 'eventra-for-wpsignal' ); ?></h2>

			<p>
				<label for="wpsignal-ks-channels">
					<?php esc_html_e( 'Channels (comma-separated):', 'eventra-for-wpsignal' ); ?>
				</label>
			...
			<p>
				<button type="button" id="wpsignal-ks-connect" class="button button-primary">
					<?php esc_html_e( 'Connect', 'eventra-for-wpsignal' ); ?>
				</button>
				<button type="button" id="wpsignal-ks-disconnect" class="button" disabled>
					<?php esc_html_e( 'Disconnect', 'eventra-for-wpsignal' ); ?>
				</button>
			...
			<div id="wpsignal-ks-event-log" style="max-height:300px;overflow-y:auto;background:#1d2327;color:#c3c4c7;padding:10px;font-family:monospace;font-size:13px;border-radius:4px;">
				<div style="color:#72aee6;"><?php esc_html_e( 'Waiting for connection...', 'eventra-for-wpsignal' ); ?></div>
			</div>
		</div>
		<?php
	}
	...
	private function render_publish_form() {
		?>
		<div class="card" style="max-width:100%;margin-top:20px;">
			<h2><?php esc_html_e( 'Publish Test Event', 'eventra-for-wpsignal' ); ?></h2>

			<table class="form-table" role="presentation">
				<tr>
					<th scope="row"><label for="wpsignal-ks-pub-channel"><?php esc_html_e( 'Channel', 'eventra-for-wpsignal' ); ?></label></th>
					<td><input type="text" id="wpsignal-ks-pub-channel" value="events" class="regular-text" /></td>
				</tr>
				<tr>
					<th scope="row"><label for="wpsignal-ks-pub-event"><?php esc_html_e( 'Event Name', 'eventra-for-wpsignal' ); ?></label></th>
					<td><input type="text" id="wpsignal-ks-pub-event" value="test.event" class="regular-text" /></td>
				</tr>
				<tr>
					<th scope="row"><label for="wpsignal-ks-pub-data"><?php esc_html_e( 'JSON Data', 'eventra-for-wpsignal' ); ?></label></th>
			...
			<p>
				<button type="button" id="wpsignal-ks-publish" class="button button-primary">
					<?php esc_html_e( 'Publish Event', 'eventra-for-wpsignal' ); ?>
				</button>
	...
	private function render_token_inspector() {
		?>
		<div class="card" style="max-width:100%;margin-top:20px;">
			<h2><?php esc_html_e( 'Token Inspector', 'eventra-for-wpsignal' ); ?></h2>

			<p>
				<button type="button" id="wpsignal-ks-mint-token" class="button">
					<?php esc_html_e( 'Mint Token', 'eventra-for-wpsignal' ); ?>
				</button>
			</p>

			<div id="wpsignal-ks-token-display" style="display:none;">
				<h3><?php esc_html_e( 'Raw Token', 'eventra-for-wpsignal' ); ?></h3>
				<textarea id="wpsignal-ks-token-raw" class="large-text" rows="3" readonly></textarea>

				<h3><?php esc_html_e( 'Decoded Claims', 'eventra-for-wpsignal' ); ?></h3>
