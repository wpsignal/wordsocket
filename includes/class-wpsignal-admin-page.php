...
class Admin_Page {
	...
	public function register_menu() {
		...
		// The submenu shares the top level's slug, so both render on this one hook.
		add_action( 'load-' . $hook, array( $this, 'enqueue_styles' ) );
	}

	/**
	 * The settings screen's stylesheet, enqueued on the page's `load-{hook}`
	 * action so it prints in `<head>`. Enqueued from `render_settings_page()`
	 * instead, it arrived as a late style in the footer, after the header had
	 * painted: the logo showed at the full width of the screen, in the admin's
	 * link blue, until the stylesheet caught up. Scripts stay in the render
	 * callback, where `wordsocket_settings_enqueue` fires.
	 *
	 * @return void
	 */
	public function enqueue_styles() {
		wp_enqueue_style(
			...
			array(),
			$this->settings_asset()['version']
		);
		wp_style_add_data( 'wpsignal-settings', 'rtl', 'replace' );
	}

	/**
	 * The settings bundle's generated asset data (dependencies and version).
	 *
	 * @return array{dependencies: string[], version: string}
	 */
	private function settings_asset() {
		$asset_file = DIR . 'build/settings.asset.php';
		return file_exists( $asset_file ) ? require $asset_file : array(
			'dependencies' => array(),
			'version'      => VERSION,
		);
	}
	...
	public function render_settings_page() {
		 ...
		 * with `$in_footer = true` is the right call. A stylesheet enqueued here
		 * prints in the footer: fine for markup the script renders, too late for
		 * anything echoed in PHP, which should enqueue on the page's load hook.
		 */
		...
		echo '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 443.21 252" width="24" height="14" aria-hidden="true" focusable="false">';
		echo '<g>';
