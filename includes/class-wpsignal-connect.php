...
/**
 * Browser-based OAuth-style connection flow.
 */
class Connect {
	...
	public function handle_start() {
		...
		set_transient( 'wpsignal_oauth_state', $state, 600 );
		...
		$connect_url = add_query_arg(
			...
			$this->config->connect_url()
		);

		$base_host = wp_parse_url( $this->config->base_url(), PHP_URL_HOST );
		// Add the base host to the allowed redirect hosts to prevent redirect loops.
		add_filter(
