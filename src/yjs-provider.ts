...
class WPSignalYjsProvider implements ProviderCreatorResult {
	...
	private init(): void {
		...
		if ( ! wps ) {
			console.error( '[WPSignal Yjs] window.WPS is not available.' );
			return;
		}
		...
		const onUpdate = ( update: Uint8Array, origin: unknown ) => {
			...
			if ( wps.connected ) {
				if ( ! this.initialized ) {
					...
					console.log( '[WPSignal Yjs] SYNC_STEP_1 sent', { channel: this.channel } );
				}
				wps.publishBinary( this.channel, this.frame( MSG_UPDATE, update ) );
				console.log( '[WPSignal Yjs] outbound update', { channel: this.channel, bytes: update.length } );
			} else {
		...
		const offBinary = wps.onBinaryMessage( ( channel, data ) => {
			...
			switch ( msgType ) {
				case MSG_SYNC_STEP_1: {
					...
					console.log( '[WPSignal Yjs] SYNC_STEP_1 received > SYNC_STEP_2 sent', {
						channel,
					...
					if ( Date.now() - this.lastSyncStep1SentAt > SYNC_STEP_1_COOLDOWN_MS ) {
						this.sendSyncStep1( wps );
						console.log( '[WPSignal Yjs] reciprocal SYNC_STEP_1 sent', { channel } );
					}
					break;
				}
				...
				case MSG_UPDATE: {
					...
					console.log( '[WPSignal Yjs] inbound update applied', {
						channel,
		...
		const offConnection = wps.onConnectionChange( ( connected ) => {
			...
			if ( connected ) {
				...
				this.sendSyncStep1( wps );
				this.initialized = true;
				console.log( '[WPSignal Yjs] connect SYNC_STEP_1 sent', { channel: this.channel } );
