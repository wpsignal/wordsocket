...
export function TabExtensions({ title }: { title: string }) {
  ...
  const [extensions, setExtensions] = useState<WordSocketExtensionInfo[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    getExtensions()
      .then((res) => setExtensions(res.extensions))
      .catch((err: any) => setError(err?.message || __("Could not load extensions.", "wordsocket")));
  }, []);

  const catalogue = (extensions ?? []).filter((ext) => !ext.installed);
  const needing = (extensions ?? []).filter((ext) => ext.installed && ext.missing.length > 0);
  ...
  return (
    <div className="wpsignal-extensions-tab">
      ...
      <p>
        {__(
          "Extensions are plugins built on WordSocket. Installed extensions add their settings here; the rest are available or on their way.",
          "wordsocket",
        )}
      </p>
      ...
      {!isConnected && (
        <Notice status="info" isDismissible={false}>
          {__("Connect your site on the Connect tab to use installed extensions.", "wordsocket")}
        </Notice>
      )}
      {needing.map((ext) => (
      ...
      {catalogue.length > 0 && (
        <div className="wpsignal-extensions-catalogue">
          ...
          <div className="wpsignal-extensions-grid">
            {catalogue.map((ext) => (
              <Card key={ext.slug} className="wpsignal-extension wpsignal-extension--catalogue" data-extension={ext.slug}>
                <CardHeader>
                  <h4>{ext.title}</h4>
                  {ext.available ? (
                    <a href={ext.docs_url} target="_blank" rel="noopener noreferrer">
                      {__("Get it", "wordsocket")}
                    </a>
                  ) : (
                    <span className="wpsignal-extension__soon">{__("Coming soon", "wordsocket")}</span>
                  )}
                </CardHeader>
                <CardBody>
                  <p>{ext.description}</p>
