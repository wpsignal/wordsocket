        ...
        ws.addEventListener("message", (e: MessageEvent) => {
          try {
            ...
            switch (msg.type) {
              ...
              case "subscribed":
                appendLog(
                  `Subscribed to: ${(msg.channels || []).join(", ")}`,
                  "#72aee6",
                );
                break;
              case "unsubscribed":
                appendLog(
                  `Unsubscribed from: ${(msg.channels || []).join(", ")}`,
                  "#ffb74d",
                );
                break;
              case "ping":
      ...
      restPost(cfg!.tokenUrl)
        ...
        .then((data: { token: string }) => {
          ...
          try {
            ...
            if (claimsEl)
              claimsEl.textContent = JSON.stringify(payload, null, 2);
            ...
            if (claimsEl)
              claimsEl.textContent = `Failed to decode: ${
